...
# OD = 2.0087 * np.array(OD1) + 0.0764   # See paper converting OD to concentration (mg/mL)
# OD = 2e8 * np.array(OD1) + 4e6   # See paper converting OD to abs. number of cells.
GFP = np.array(GFP1) * 1e-4    # Smaller than that and the fitting is very wierd
# GFP = np.array(GFP1) * 1e-8     # Horrible estimate (mine, fig.1) on paper Toolbox for exploring modular ... by Magaraci et al.
...
# %% Paramaters (normalised) from literature --> Ignore ...
...
t1 = time.time()
p_ = stoch_and_ODE_RK4(fitted_params, N0=OD[0], t0=0, tf = 0,
...
gamma_ex = gamma_x
# c_sec *= 3
fitted_params_new = (c_sec, gamma_ex, A_syn, V_min, V_max, K, gamma_x, r, carr_k)
...
# p_ = stoch_and_ODE_RK4(fitted_params_new, N0=OD[0], t0=0, tf = 40, dt=0.005, self_simulation=True)

# %%


from Find_bif_param import bifurcation_value
K_star = bifurcation_value(fitted_params_new)
K_new = 0.75 * K_star
fitted_params_new = (c_sec, gamma_ex, A_syn, V_min, V_max, K_new, gamma_x, r, carr_k)

# %%
from Finding_fixed_master import eq_pis
all_pis = eq_pis(fitted_params_new)

# %% Same same but fixed params
from Find_bif_param import bifurcation_value
K_star = bifurcation_value(fitted_params_new)
...
from Group_sensing import stoch_and_ODE_RK4
t1 = time.time()
# p_ = stoch_and_ODE_RK4(fitted_params, tf=13, dt = 0.00001)
p_ = stoch_and_ODE_RK4(fitted_params_new, N0=OD[0], t0=0, tf = 100, dt=0.005, self_simulation=True)
...
t1 = time.time()
p_ = stoch_and_ODE_RK4_3(fitted_params_new, N0=OD[0], t0=0, tf = 15,
...
t1 = time.time()
# p_ = stoch_and_ODE_RK4(fitted_params, tf=13, dt = 0.00001)
p_ = stoch_and_ODE_RK4(fitted_params_new, N0=OD[0], t0=0, tf = 27,
...
p_ = stoch_and_ODE_RK4_2(0.27, fitted_params_new, N0=OD[0], t0=0, tf = 100,
                         dt=0.01, self_simulation=True)
t2 = time.time()
...
t1 = time.time()
stoch_and_ODE_RK4_2(0.27, fitted_params_new, N0=0.07683966, t0=0, tf = 30, dt=0.01, self_simulation=True)
t2 = time.time()
...
# %%
"""
# More old files, probably not needed!!
# %% Stochastic Model
from Gillespie_1 import gillespie_1


# Simulate 25 steps of the trajectory of X starting from 250 molecules (rescaling completely wrong, bu code works).
gillespie_1(fitted_params)


# %% NOTE! K is normally distributed around mean (fitted) K. Variance?
from Gillespie_mult import gillespie_mult
# Simulate (100 simulations) for 25 steps of the trajectory of X starting from 250 molecules (rescaling completely wrong, bu code works).
gillespie_mult(fitted_params2)

# %% NOTE! K is normally distributed around mean (fitted) K. Variance? 
from Gillespie_tau_leaping import gillespie_tau_leaping
# Simluate 25 steps of the trajectory of X starting from 250 molecules (rescaling completely wrong, bu code works).
gillespie_tau_leaping(fitted_params2)


# %%

if False:
    plt.plot(exp_t1, np.array(OD1), 'r*')
    plt.plot(exp_t2, np.array(OD2), 'b*')
    plt.plot(exp_t3, np.array(OD3), 'g*')
    plt.plot(exp_t1, np.array(OD1), 'r-', label="1")
    plt.plot(exp_t2, np.array(OD2), 'b-', label="2")
    plt.plot(exp_t3, np.array(OD3), 'g-', label="3")
    plt.title("OD --- Mutants: ∆qslA ∆qteE")
    plt.legend(loc='best')
    plt.xlabel("Time")
    plt.ylabel("Luminosity")
    plt.show()
    
    plt.plot(exp_t_WT1, np.array(OD_WT1), 'r*')
    plt.plot(exp_t_WT2, np.array(OD_WT2), 'b*')
    plt.plot(exp_t_WT3, np.array(OD_WT3), 'g*')
    plt.plot(exp_t_WT1, np.array(OD_WT1), 'r-', label="1")
    plt.plot(exp_t_WT2, np.array(OD_WT2), 'b-', label="2")
    plt.plot(exp_t_WT3, np.array(OD_WT3), 'g-', label="3")
    plt.title("OD --- Wild types")
    plt.xlabel("Time")
    plt.ylabel("Luminosity")
    plt.show()
    
    plt.plot(exp_t1, np.array(GFP1), 'r*')
    plt.plot(exp_t2, np.array(GFP2), 'b*')
    plt.plot(exp_t3, np.array(GFP3), 'g*')
    plt.plot(exp_t1, np.array(GFP1), 'r-', label="1")
    plt.plot(exp_t2, np.array(GFP2), 'b-', label="2")
    plt.plot(exp_t3, np.array(GFP3), 'g-', label="3")
    plt.title("Median GFP --- Mutants: ∆qslA ∆qteE")
    plt.legend(loc='best')
    plt.xlabel("Time")
    plt.ylabel("Luminosity")
    plt.show()
    
    plt.plot(exp_t_WT1, np.array(GFP_WT1), 'r*')
    plt.plot(exp_t_WT2, np.array(GFP_WT2), 'b*')
    plt.plot(exp_t_WT3, np.array(GFP_WT3), 'g*')
    plt.plot(exp_t_WT1, np.array(GFP_WT1), 'r-', label="1")
    plt.plot(exp_t_WT2, np.array(GFP_WT2), 'b-', label="2")
    plt.plot(exp_t_WT3, np.array(GFP_WT3), 'g-', label="3")
    plt.title("Median GFP --- Wild types")
    plt.xlabel("Time")
    plt.ylabel("Luminosity")
    plt.show()
