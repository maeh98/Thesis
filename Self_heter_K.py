...
# -*- coding: utf-8 -*-
"""
Created on Sun May  8 00:49:26 2022
...
def Markov_ODE_system(p, t, *args):  # , *args
    ...
    for j, K_temp in enumerate(K_values):
        args = (c_sec, gamma_ex, A_syn, V_min, V_max, K_temp, gamma_x, r, carr_k, N0, M, zeta, Se)
        prod_vec = np.array([Prod2(j, rho_v, args) for j in states])
        M = int(M)
        dpdt[j * M] = gamma_x * p[1 + j * M] - zeta * V_min * p[j * M]
        dpdt[1 + j * M: -1 + (j + 1) * M] = - (gamma_x * states[1: -1] + prod_vec[1: -1]) * p[1 + j * M: -1 + (j + 1) * M] + (gamma_x * states[2: ]) * p[2 + j * M: (j + 1) * M] + prod_vec[:-2] * p[j * M: -2 + (j + 1) * M]
        dpdt[(j + 1) * M - 1] = - (gamma_x * states[-1] + prod_vec[-1]) * p[(j + 1) * M - 1] + prod_vec[-2] * p[(j + 1) * M - 2]
...
# %%
def stoch_and_ODE_RK4_2(Se, fitted_params, N0=0.07683966, t0=0, tf = 13, dt=0.01, self_simulation=True):
    ...
    for j in np.arange(5):
        plt.plot(states, p_[times[0], j, :M], 'g', label='time 0')
        plt.plot(states, p_[times[1], j, :M], 'y', label=f'time {times[1] * dt}')
        plt.plot(states, p_[times[2], j, :M], 'r', label=f'time {times[2] * dt}')
        plt.plot(states, p_[times[3], j, :M], 'm', label=f'time {times[3] * dt}')
        plt.plot(states, p_[times[4], j, :M], 'k', label=f'time {times[4] * dt}')
        plt.plot(states, p_[-1, j, :M], 'b*', label=f'time {times[5] * dt}')
        plt.ylim([0, 0.025])
    ...
    plt.plot(states, p_mean[times[0], :M], 'g', label='time 0')
    plt.plot(states, p_mean[times[1], :M], 'y', label=f'time {times[1] * dt}')
    plt.plot(states, p_mean[times[2], :M], 'r', label=f'time {times[2] * dt}')
    plt.plot(states, p_mean[times[3], :M], 'm', label=f'time {times[3] * dt}')
    plt.plot(states, p_mean[times[4], :M], 'k*', label=f'time {times[4] * dt}')
    plt.plot(states, p_mean[-1, :M], 'b*', label=f'time {times[5] * dt}')
    plt.ylim([0, 0.025])
    plt.title('Probability distribution over time -- Group Sensing', fontweight='bold')
    plt.xlabel("states", fontweight='bold')
    plt.ylabel('probability', fontweight='bold')
